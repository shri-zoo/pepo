...
2. Запускаем `npm run nodemon:ssl`. В первый раз он вам предложит добавить `127.0.0.0    pepo.local` в `/etc/hosts` и создаст ssl-сертификаты для сервера, вам остается лишь понажимать Enter когда команда будет делать запросы на ввод данных.
