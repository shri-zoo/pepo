...
#### Запуск gemini тестов
Тестовые данные блока кладутся в файл ```block/block.tests/gemini.bemjson.js``` (см. для примера ```common.blocks/username/username.tests/gemini.bemjson.js```).

Gemini тесты кладутся в ```gemini/block.gemini.js``` (см. для примера
```gemini/username.gemini.js```)

Запуск тестов, одновременно в трёх консолях:
1. Запуск ```sc```:
```sh
SOUCE_USERNAME=X SOUCE_ACCESS_KEY=K bin/sc
```
2. Запуск ```magic server``` для отдачи статики тестов:
```sh
npm run magic
```
3. Запуск ```gemini-gui```
```sh
gemini-gui [gemini/block.gemini.js]
```
